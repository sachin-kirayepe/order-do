...
interface OrderModalProps {
  ...
  onUpdateStatus: (orderId: string, status: 'pending' | 'accepted' | 'rejected' | 'ready') => Promise<void>;
}

export default function OrderModal({ order, onClose, onDone, onReject, onUpdateStatus }: OrderModalProps) {
  const { hasFeature } = useSubscription();
  const customerName = (order as any).decryptedName || 'Customer';
  const orderStatus = order.status || 'pending';
  const customerAddress = (order as any).decryptedAddress || 'Address';
  ...
  // Security Checks
  const isPurged = customerName.includes('AUTO_PURGED');

  const handleReveal = () => {
    if (isPurged) {
      toast.error(language === 'hi' ? 'Purana data delete ho chuka hai!' : 'Old data has been purged!');
      return;
    }
    toast.warning(language === 'hi'
      ? 'Security Alert: Aap grahak ka data dekh rahe hain. Ye action record ho raha hai!'
      : 'Security Alert: You are viewing personal data. This action is being monitored!', {
      duration: 5000,
  ...
  const handleWhatsApp = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!isRevealed) {
      toast.info(language === 'hi' ? 'Pehle "Unlock" button dabayein' : 'Please click "Unlock" first');
      return;
    }

    if (isPurged || !order.customerPhone) return;
    ...
    // Blind Redirect: Generator link only at the point of action
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-slate-100 dark:border-slate-700 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasFeature('whatsapp_alerts') && order.customerPhone && (
              <button
                onClick={handleWhatsApp}
                className={`p-2 rounded-lg transition-all ${isRevealed
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <SecureCanvas content={isRevealed ? customerName : '[HIDDEN_NAME]'} width={220} height={32} fontSize={18} className="border-none bg-transparent" />
                {order.type && order.no && (
              ...
              {/* Masked/Revealed Details */}
              <div className="space-y-1.5 mt-1">
                <div className="flex items-center gap-2">
                  <SecureCanvas content={isRevealed ? customerAddress : '[HIDDEN_ADDRESS]'} width={220} height={20} fontSize={12} className="border-none bg-transparent" />
                </div>

                {order.customerPhone && (
                  <div className={`flex items-center gap-2 py-1 px-2 rounded-lg border w-fit transition-all ${isRevealed ? 'bg-blue-50 border-blue-100' : 'bg-slate-100 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600'
                    }`}>
                    <span className="text-[10px] font-mono font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                      +91 {isRevealed ? order.customerPhone : maskPhone(order.customerPhone)}
                    </span>
                    <div className={`w-1 h-1 rounded-full animate-pulse ${isRevealed ? 'bg-blue-500' : 'bg-kirana-green'}`} />
                    <span className={`text-[8px] font-black uppercase tracking-tighter ${isRevealed ? 'text-blue-500' : 'text-kirana-green'}`}>
                      {isRevealed ? 'Revealed' : 'Protected'}
                    </span>
                  </div>
                )}
              </div>
              ...
              {!isRevealed && (
                <button
                  ...
                  className="mt-3 flex items-center gap-2 px-3 py-1.5 bg-kirana-green text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg shadow-kirana-green/20 hover:scale-105 active:scale-95 transition-all"
                >
                  <Lock size={12} /> Unlock Info & WhatsApp
                </button>
              )}

              <p className="text-[9px] text-slate-300 mt-2 uppercase tracking-wider font-bold italic">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all cursor-pointer ${paymentReceived
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 px-3 py-2 bg-blue-500/5 dark:bg-blue-500/10 rounded-xl border border-blue-500/20">
                <ShieldCheck size={14} className="text-blue-500" />
                <p className="text-[9px] font-bold text-blue-600 dark:text-blue-400 leading-tight uppercase tracking-widest">
                  Privacy Shield Active: Customer information will be permanently erased after order completion.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
             <div className="flex items-center justify-between px-4 py-3 bg-slate-100/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Workflow Stage</span>
                <span className="text-[10px] font-black text-brand-primary uppercase tracking-widest animate-pulse">{orderStatus.toUpperCase()}</span>
             </div>
             <div className="p-2 grid grid-cols-4 gap-2">
                {[
                  { id: 'accepted', icon: <CheckCircle2 size={16} />, label: language === 'hi' ? 'हाँ' : 'Accept', color: 'bg-blue-500' },
                  { id: 'preparing', icon: <UtensilsCrossed size={16} />, label: language === 'hi' ? 'तैयार' : 'Cook', color: 'bg-kirana-orange' },
                  { id: 'ready', icon: <Sparkles size={16} />, label: language === 'hi' ? 'तैयार है' : 'Ready', color: 'bg-kirana-green' },
                  { id: 'completed', icon: <ShieldCheck size={16} />, label: language === 'hi' ? 'पूरा' : 'Done', color: 'bg-slate-900' }
                ].map((s) => (
                  <button
                    key={s.id}
                    onClick={() => s.id === 'completed' ? handleDone() : onUpdateStatus(order.id, s.id as 'pending' | 'accepted' | 'rejected' | 'ready')}
                    className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all border-2 ${
                      orderStatus === s.id 
                        ? `${s.color} border-${s.id} text-white shadow-lg scale-105` 
                        : 'bg-white dark:bg-slate-800 border-transparent text-slate-400 opacity-60 hover:opacity-100 hover:border-slate-200 dark:hover:border-slate-700'
                    }`}
                  >
                    {s.icon}
                    <span className="text-[8px] font-black uppercase tracking-tighter mt-1">{s.label}</span>
                  </button>
                ))}
             </div>
          </div>
