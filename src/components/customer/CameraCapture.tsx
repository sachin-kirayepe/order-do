...
export default function CameraCapture({ shopId, onCapture }: CameraCaptureProps) {
  ...
  // Voice Guidance (Free Native API)
  const speak = useCallback((text: string) => {
    if (!showVoice || !window.speechSynthesis) return;
  ...
  const processVideoFrame = async () => {
    ...
    const detected = detectedFaces.length > 0;
    setIsFaceDetected(detected);
    setFaceCount(detectedFaces.length);

    // Environment analysis (every 30 frames for performance)
    if (performance.now() - lastEnvCheckRef.current > 1000) {
      ...
      if (ctx) {
        ...
        setEnvFeedback({ isTooDark: analysis.isTooDark, isTooBright: analysis.isTooBright, isBlurry: analysis.isBlurry });
        lastEnvCheckRef.current = performance.now();
      }
    }
    ...
    if (detected) {
      ...
      // Face Distance/Position Check
      const faceWidth = Math.abs(landmarks[454].x - landmarks[234].x);
      const isIdeal = faceWidth > 0.25 && faceWidth < 0.6;
      setIsPositionIdeal(isIdeal);

      if (faceWidth < 0.25) {
        setTip('कैमरा के थोड़ा पास आएं');
      } else if (faceWidth > 0.6) {
        setTip('थोड़ा दूर हटें');
      } else {
        setTip(null);
      }
  ...
  const detectBlink = (landmarks: any[]) => {
    ...
    if (ear < 0.15 && lastEar >= 0.15) {
      vibrate(40);
      setBlinkCount(prev => {
        const next = prev + 1;
        if (next === 1) speak('बहुत बढ़िया, एक और बार');
        if (next >= 2) {
          vibrate([50, 30, 50]);
          setLivenessStep('success');
          speak('सत्यापन सफल, फोटो ली जा रही है');
        }
        return next;
      });
    }
  ...
  useEffect(() => {
    if (hasConsent && livenessStep === 'blink' && !loading) {
      speak('नमस्ते, कृपया अपनी आँखें झपकाएं');
    }
  }, [hasConsent, loading]);
    ...
    <div className="flex flex-col items-center gap-4 relative">
      ...
      {!captured ? (
        <div className="w-full flex flex-col items-center gap-4">
          <div className="relative w-full max-w-sm aspect-[4/3] bg-slate-900 rounded-3xl overflow-hidden shadow-2xl border-4 border-white/10 group">
            {loading && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-slate-950 text-white">
                ...
                <div className="text-sm font-bold opacity-50">Starting Camera...</div>
              </div>
            )}
            ...
            <div className="absolute inset-0 pointer-events-none z-10 flex items-center justify-center">
              <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                ...
                <ellipse
                  ...
                  strokeWidth="0.5"
                  className={`transition-all duration-500 ${isPositionIdeal ? 'animate-pulse' : ''}`}
                />

                {/* Cyber-Security Biometric Dots */}
                ...
                {!captured && !loading && (
                  <motion.rect
                    ...
                    animate={{ opacity: [0, 0.2, 0], y: [10, 80, 10] }}
            ...
            <div className="absolute inset-0 pointer-events-none z-20 flex flex-col items-center justify-between p-6">
              <AnimatePresence mode="wait">
                ...
                >
                  <div className={`p-4 rounded-2xl backdrop-blur-xl border-2 transition-all duration-500 ${livenessStep === 'blink' ? 'bg-kirana-green/30 border-kirana-green/50 shadow-lg shadow-kirana-green/20' : 'bg-black/60 border-white/10 opacity-30 scale-95'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-white text-base font-bold flex items-center gap-3">
                        <div className={`p-1.5 rounded-lg ${blinkCount >= 2 ? 'bg-kirana-green text-white' : blinkCount > 0 ? 'bg-yellow-500 text-white' : 'bg-white/20 text-white'}`}>
                          {blinkCount >= 2 ? <CheckCircle size={18} /> : <Eye size={18} />}
                        </div>
                  ...
                  {/* Movement UI hidden as per simplification */}
                </motion.div>
              </AnimatePresence>
              ...
              <AnimatePresence>
                {(tip || livenessStep === 'success' || faceCount > 1 || envFeedback.isTooDark || envFeedback.isBlurry) && (
                  <motion.div
                    ...
                    className={`px-6 py-3 rounded-2xl font-bold flex items-center gap-3 shadow-2xl backdrop-blur-2xl ${faceCount > 1 || envFeedback.isTooDark
                  ...
                  >
                    {faceCount > 1 ? (
                      ...
                      <><Zap size={20} className="text-yellow-400" /> {language === 'hi' ? 'रोशनी कम है' : 'Too dark, add light'}</>
        ...
        <>
          ...
          <p className="text-sm text-kirana-green font-semibold flex items-center gap-1">
            <CheckCircle size={16} /> {t('customer.photoTaken')}
          </p>
          <button
            onClick={retake}
            className="flex items-center gap-2 text-sm text-slate-500 hover:text-red-500 transition-colors"
          >
            <RefreshCcw size={15} /> {language === 'hi' ? 'दोबारा लें' : 'Retake'}
          </button>
        </>
      )}
    </div>
